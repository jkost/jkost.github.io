...
**Εικόνα 3.5.1** _Ενθυλάκωση στον αντικειμενοστραφή προγραμματισμό_
...
Ας δούμε πώς θα έπρεπε να είχαμε γράψει τη κλάση ```Car``` κάνοντας σωστή χρήση της ενθυλάκωσης.
...
Πλέον έχουμε ενθυλακώσει τα γνωρίσματα της κλάσης ορίζοντάς τα ως ```private```, που σημαίνει ότι είναι προσβάσιμα μόνο από αντικείμενα της κλάσης ```Car```, και την κλάση και τις μεθόδους της ως ```public``` πράγμα που σημαίνει ότι είναι προσβάσιμες από οποιαδήποτε άλλη κλάση του προγράμματος.
...
jshell> car
car ==> Car {model=Kia Ceed, maxSpeed=210, ccm=1353, speed=10}

jshell> Car otherCar = car;
otherCar ==> Car {model=Kia Ceed, maxSpeed=210, ccm=1353, speed=10}
...
jshell> otherCar
otherCar ==> Car {model=Kia Ceed, maxSpeed=210, ccm=1353, speed=0}

jshell> car
car ==> Car {model=Kia Ceed, maxSpeed=210, ccm=1353, speed=0}
```
...
```
διαφορετικά αν δεν χρησιμοποιήσουμε την ```import``` θα πρέπει να γράψουμε π.χ.:
```java
...
**Εικόνα 3.5.10** _Παράδειγμα διαγράμματος αρθρωμάτων στη UML_
