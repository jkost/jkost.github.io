...
6) Ο [τύπος του Ήρωνα](https://el.wikipedia.org/wiki/Τύπος_του_Ήρωνα) δίνει το εμβαδόν ενός τριγώνου συναρτήσει του μήκους των πλευρών του $$\large E=\sqrt{s(s-a)(s-b)(s-c)}$$ <br>όπου ```s``` είναι η ημιπερίμετρος του τριγώνου $$\Large s=\frac{a+b+c}{2}$$. Να γραφτεί ένα πρόγραμμα που να τυπώνει το εμβαδόν ενός τριγώνου με πλευρές ```a```, ```b```, ```c``` χρησιμοποιώντας τον τύπο του Ήρωνα. _Συμβουλή: Θα χρεαστείτε την ```Math.sqrt()``` για να υπολογίσετε την τετραγωνική ρίζα ενός αριθμού._
