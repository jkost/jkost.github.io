...
Η σύνταξη μετατροπής σε έναν άλλον τύπο δεδομένων (casting) γίνεται δηλώνοντας τον νέο τύπο δεδομένων μέσα σε παρενθέσεις μπροστά από το δεδομένο. Στο παρακάτω παράδειγμα βλέπουμε πώς μπορεί να χαθεί ακρίβεια μετατρέποντας έναν ακέραιο αριθμό σε ```byte``` (το ```byte``` μπορεί να πάρει τιμές από το ```-128``` μέχρι το ```127```, βλ. πίνακα 1.6.1): 
...
**Επεξήγηση του quiz ```a + b```:** Η μεταβλητή ```a``` τύπου ```char``` μετατρέπεται κι αυτή σε ```int```, δηλ. στην ψηφιακή αναπαράστασή της σε μορφή ASCII. Ο κωδικός ASCII του χαρακτήρα ```'1'``` είναι ο αριθμός ```49```, επομένως, ```49+1=50```. 
