...
Στη συνέχεια, δώστε μέσα στο φάκελο του έργου σας όπου βρίσκεται το ```build.xmll``` την εντολή:

```bash
$ ant
```
και η εντολή θα οικοδομήσει το έργο σας διαβάζοντας το ```build.xml``` αρχείο.
...
```
Τέλος, σα ζητά να επιβεβαιώσετε τις τιμές των ιδιοτήτων. Πατήστε ```Enter```.
```bash
