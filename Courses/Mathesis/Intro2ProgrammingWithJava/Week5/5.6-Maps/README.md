...
Αν και η κατάσταση δε βελτιώθηκε, μια καλή υλοποίηση της ```hashCode()``` που κατανέμει σωστά τα στοιχεία στους κουβάδες του πίνακα κατακερματισμού είναι πολύ σημαντική και πολλές φορές πηγή σφαλμάτων. Δυστυχώς, είναι κι ένα πολύ δύσκολο πρόβλημα.
...
Η μέθοδος `Objects.hash()` πολλαπλασιάζει αυξητικά την τρέχουσα τιμή με τον πρώτο αριθμό 31 προτού προσθέσει τη νέα τιμή κάτι το οποίο δουλεύει πολύ καλά αν το κλειδί είναι τύπου `String`. Επειδή χρησιμοποιούμε το λατινικό αλφάβητο (που έχει το πολύ μέχρι 26 χαρακτήρες), πολλαπλασιάζοντας με το 31 είναι αρκετό για να μειώσουμε τον αριθμό των συγκρούσεων.

**Σημείωση** _Μπορείτε να χρησιμοποιήσετε την κλάση [MapClashInspector](https://www.javaspecialists.eu/archive/Issue235-Checking-HashMaps-with-MapClashInspector.html) για να δείτε πόσες συγκρούσεις (collisions) έχετε στον πίνακα κατακερματισμού σας και κατά συνέπεια κατά πόσο καλή είναι η υλοποίηση της `hashCode()` σας_.

Από την έκδοση 8 και μετά, η υλοποίηση της `HashMap` αποθηκεύει τις συγκρούσεις (collisions) σε ένα δέντρο για γρήγορη αναζήτηση, οπότε καλό είναι μαζί με την `hashCode()` η κλάση σας, αντικείμενα της οποίας θα αποθηκευθούν σ' ένα `HashMap`, να υλοποιεί (`implements`) και την `Comparable` ώστε να βοηθήσετε στην πιο γρήγορη αναζήτηση στο δέντρο σε περίπτωση συγκρούσεων.
