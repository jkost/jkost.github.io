...
Χρησιμοποιήσαμε έναν βρόγχο ```while``` γιατί δεν γνωρίζουμε εκ των προτέρων πόσες επαναλήψεις θα χρειαστούν. Σε κάθε επανάληψη ελέγχουμε κατά πόσο η μεταβλητή ```c``` η οποία κάθε φορά αυξάνεται με το επιτόκιο (```c *= interest <=> c = c * interest```). Παρόλ' αυτά, για πιο προχωρημένους, θα μπορούσαμε να χρησιμοποιήσουμε και βρόγχο ```for``` ως εξής (και υπάρχουν κι άλλοι συνδυασμοί που ξεφεύγουν από αυτό το εισαγωγικό μάθημα):

```java
jshell> var c = capital;
c ==> 10000.00

jshell> for (; c < 2*capital; years++) {
   ...>     c *= interest;
   ...> }
...
```
Βγάλαμε την αρχικοποίηση της μεταβλητής ```c``` έξω από τον βρόγχο επειδή τη χρειαζόμαστε μετά το τέλος του βρόγχου. Αν την ορίζαμε μέσα στη ```for``` όπως παρακάτω, δεν θα μπορούσαμε να την προσπελάσουμε καθώς η εμβέλειά της θα ήταν μέχρι το }.

```java
jshell> for (var c = capital; c < 2*capital; years++) {
   ...>     c *= interest;
   ...> }
...
Ας δούμε την άσκηση 7. Είναι μια δύσκολη άσκηση. Θα τη δοκιμάσουμε στο [OneCompiler](https://onecompiler.com/jshell/) ή στο NetBeans. Ξεκινάμε με τρεις φυσικούς αριθμούς ```a, b, c``` για τους οποίους ισχύει ```a < b < c```, π.χ. ```a = 0, b = 1, c = 2```. Στη συνέχεια πρέπει να βρούμε την τριάδα για την οποία ισχύει ```a&sup;2 + b&sup;2 = c&sup;2``` και  ```a + b + c = 1000```, και να υπολογίσουμε το γινόμενο της ```a*b*c```, δηλ.
