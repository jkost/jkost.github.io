...
Ας δούμε την άσκηση 7. Είναι μια δύσκολη άσκηση. Θα τη δοκιμάσουμε στο [OneCompiler](https://onecompiler.com/jshell/) ή στο NetBeans. Ξεκινάμε με τρεις φυσικούς αριθμούς ```a, b, c``` για τους οποίους ισχύει ```a < b < c```, π.χ. ```a = 0, b = 1, c = 2```. Στη συνέχεια πρέπει να βρούμε την τριάδα για την οποία ισχύει ```a&sup;2 + b&sup;2 = c&sup;2``` και  ```a + b + c = 1000```, δηλ.
