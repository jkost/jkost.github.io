...
ο μεταγλωττιστής θα παραπονεθεί καθώς η ```Engine``` δεν μπορεί να κληθεί από μια στατική μέθοδο όπως είναι η ```main()```. Το λάθος διορθώνεται εύκολα αν ορίσουμε την εσωτερική κλάση ```Engine``` ως ```static```:
...
car ==> Car@589838eb
```
Ορίζοντας την ```Engine``` ως μη ```static``` όμως:
```java
jshell> Car car = new Car("Audi A3", 210, new Car.Engine(1984, 6000, 170));
|  Error:
...
|        at (#5:1)
```
Στην περίπτωση αυτή, χρειαζόμαστε ένα αντικείμενο τύπου ```Car``` για να μπορέσουμε να δημιουργήσουμε ένα αντικείμενο τύπου ```Engine```. Καθώς όμως το αντικείμενο τύπου ```Car``` δεν έχει δημιουργηθεί ακόμα (καλούμε τον constructor του), η ```car.new Engine(...)``` επιστρέφει ```NullPointerException```. Στην περίπτωση αυτή, θα πρέπει να δημιουργήσουμε ένα αντικείμενο τύπου ```Car``` χωρίς μηχανή και στη συνέχεια να προσθέσουμε μια μηχανή με μια μέθοδο π.χ. ```addEngine(Engine engine)``` (αφήνεται ως άσκηση για τον αναγνώστη).

Χρησιμοποιήστε στατικές εμφωλιασμένες κλάσεις όταν θέλετε η κατάσταση (state) που αποθηκεύουν να είναι προσβάσιμη κι από εξωτερικές κλάσεις. Διαφορετικά χρησιμοποιήστε εσωτερικές (inner) κλάσεις.

Μπορούμε να δημιουργήσουμε και ανώνυμες εσωτερικές κλάσεις, αλλά θα τις περιγράψουμε στο μάθημα για την κληρονομικότητα.
...
**Εικόνα 2** _Παράδειγμα διαγράμματος βιβλιοθηκών στη UML_
...
**Εικόνα 3** _Περιγραφέας αρθρώματος (module descriptor)_
...
**Εικόνα 4** _Το JDK 9 τμηματοποιημένο σε αρθρώματα_

Το JDK 9 έχει τμηματοποιηθεί σε 19 αρθρώματα. Το άρθρωμα ```java.base``` είναι το βασικό άρθρωμα που εισάγεται εξ' ορισμού σε όλα τα προγράμματα Java. Εκθέτει διάφορες βασικές βιβλιοθήκες όπως ```java.lang``` και ```java.util```. Από εκεί και πέρα μπορείτε να προσθέσετε μόνο τα αρθρώματα που χρειάζεστε για την εφαρμογή σας. Π.χ. αν χτίζετε μια εφαρμογή desktop, προσθέστε μια εξάρτηση στο άρθρωμα ```java.desktop``` κλπ.

To Apache NetBeans υποστηρίζει modules.

![](assets/Fig5.png)

**Εικόνα 5** _Δημιουργία αρθρώματος με το NetBeans_

![](assets/Fig6.png)

**Εικόνα 6** _Γραφική αναπαράσταση εξαρτήσεων από το NetBeans_
...
**Εικόνα 7** _Απαγορεύονται οι κυκλικές εξαρτήσεις μεταξύ αρθρωμάτων_
...
**Εικόνα 8** _Παράδειγμα διαγράμματος αρθρωμάτων στη UML_
