...
### Αντιγραφή 
Ήδη είδαμε τον copy constructor ```HashSet(Collection<?> c)``` που δημιουργεί ένα νέο ```HashSet``` από τη συλλογή που του περνάμε ως όρισμα. Υπάρχει επίσης και η στατική μέθοδος ```copyOf(Collection<? extends E> c)```.
