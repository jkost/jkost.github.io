...
| ```float``` | 4 bytes | ```-3.40282347E+38``` (```Float.MIN_VALUE```) -> ```-1.4E-45``` και ```1.4E-45 -> 3.4028235E+38``` (```Float.MAX_VALUE```) |
| ```double``` | 8 bytes | ```-1.7976931348623157E+308``` (```Double.MIN_VALUE```) -> ```-4.9E-324``` και ```4.9E-324 -> 1.7976931348623157E+308``` (```Double.MAX_VALUE```) |
...
Στο δεύτερο παράδειγμα χρησιμοποιούμε τον προθεματικό τελεστή προσαύξησης. Ως αποτέλεσμα πρώτα προσαυξάνεται η τιμή της ```x``` κατά ```1``` (```3```) και στη συνέχεια πολλαπλασιάζεται με το ```10``` και αποθηκεύεται στην μεταβλητή ```answer```.
...
**Πίνακας 1.6.6**  _Τύπος δεδομένων χαρακτήρας_
...
**Πίνακας 1.6.7** _Πράξεις σε χαρακτήρες_
...
**Πίνακας 1.6.8** _Προτεραιότητα τελεστών_
...
Σαν άσκηση, επιλήστε μόνοι σας την αντίστροφη μετατροπή, δηλ. τη μετατροπή από βαθμούς κελσίου σε βαθμούς φαρενάιτ. Παρόμοια λύνεται και η άσκηση 6. 
