...
```
Το πρόγραμμα δεν είναι καθολικό όπως θα ήθελα και για να το κάνω θα χρειαστεί πολλές ακόμα γραμμές κώδικα, κάτι που ξεφεύγει από το σκοπό αυτού του άρθρου. Παραδείγματα υλοποίησης του κάθε αλγόριθμου θα βρείτε στην αναφορά [8] και σε τούτη την ιστοσελίδα.
...
**Σχήμα 4** _Λειτουργία του αλγορίθμου δημοσίου κλειδιού_ 
...
Το πρόγραμμα ```SimpleASymmetricExample```, το οποίο μπορείτε να κατεβάσετε από εδώ, επιδεικνύει ένα παράδειγμα κρυπτογραφίας δημόσιου κλειδιού. Το πρόγραμμα μοιάζει πολύ με αυτό της συμμετρικής κρυπτογραφίας SimpleSymmetricExample του προηγούμενου τεύχους, το οποίο μπορείτε επίσης να κατεβάσετε από [εδώ](assets/java_cryptography.zip).
...
1. Η Αλίκη επιλέγει ένα τυχαίο μεγάλο ακέραιο ```α∈[1, p-1)```, υπολογίζει το ```gα mod p``` (το δημόσιο κλειδί της) και στέλνει το αποτέλεσμα στο Βασίλη.
2. Ο Βασίλης επιλέγει ένα τυχαίο μεγάλο ακέραιο ```β∈[1, p-1)```, υπολογίζει το ```gβ mod p``` (το δημόσιο κλειδί του) και στέλνει το αποτέλεσμα στην Αλίκη.
3. Η Αλίκη υπολογίζει το ```k ← (gβ)α mod p```
4. Ο Βασίλης υπολογίζει το ```k ← (gα)β mod p```
...
1. Η Αλίκη επιλέγει ένα τυχαίο μεγάλο ακέραιο ```α∈R[1, p-1)```, υπολογίζει το ```gα mod p``` (το δημόσιο κλειδί της) και στέλνει το αποτέλεσμα στην Εύα («Βασίλη»).
1’. Η Εύα («Αλίκη») υπολογίζει το ```gε = gε mod p``` για κάποιο ```ε∈R[1, p-1)``` και το στέλνει στο Βασίλη.
2. Ο Βασίλης επιλέγει ένα τυχαίο μεγάλο ακέραιο ```β∈[1, p-1)```, υπολογίζει το ```gβ mod p``` (το δημόσιο κλειδί του) και στέλνει το αποτέλεσμα στην Εύα («Αλίκη»).
2’.  Η Εύα («Βασίλης») στέλνει το ```gε``` στην Αλίκη
3. Η Αλίκη υπολογίζει το ```k1 ← (gε)α mod p``` (κοινό μυστικό μεταξύ αυτής και της Εύας)
4. Ο Βασίλης υπολογίζει το ```k2 ← (gε)β mod p``` (κοινό μυστικό μεταξύ του και της Εύας)

![](assets/Fig6.png)

Σχήμα 6 Man-in-the-middle attack στον αλγορίθμο Diffie-Hellman
...
**Σχήμα 7** _Λειτουργία της ψηφιακής υπογραφής_
...
**Σχήμα 8** _Man-in-the-middle (ενδιάμεσος)_
