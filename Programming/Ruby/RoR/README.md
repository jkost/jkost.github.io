...
Rails 3.2.8
```
(_Η Ruby 1.9.1 ενδέχεται να σας δημιουργήσει προβλήματα με τη RoR, γι’ αυτό είτε αναβαθμιστείτε στην 1.9.2 είτε χρησιμοποιήστε την 1.8.7_). Η πιο
...
και φυσικά οι αγαπημένοι σας κειμενογράφοι [vim](https://www.vim.org/scripts/script.php?script_id=1567) και [emacs](https://www.emacswiki.org/emacs/RubyOnRails) διαθέτουν πρόσθετα για υποστήριξη RoR. Ένα ΟΠΕ μπορεί ν’ αποβεί ιδιαίτερα χρήσιμο, πέραν των άλλων, κατά την αποσφαλμάτωση της εφαρμογής σας (όταν π.χ. ψάχνετε να βρείτε γιατί η ```params([:action])``` δεν επιστρέφει αυτό που περιμένατε).
